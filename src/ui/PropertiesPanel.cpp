...
PropertiesPanel::PropertiesPanel(wxWindow* parent)
    : wxPanel(parent, wxID_ANY), m_title(nullptr), m_details(nullptr) {
    ...
    m_title->SetFont(wxFont(11, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL,
                            wxFONTWEIGHT_BOLD));
...
void PropertiesPanel::SetInfo(const wxString& title, const wxString& details) {
    ...
    if (m_details) m_details->SetLabel(details);
}
