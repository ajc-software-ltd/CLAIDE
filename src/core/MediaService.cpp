...
const std::set<std::string> kVideoExtensions = {
...
const std::set<std::string> kModelExtensions = {
    ".fbx", ".obj", ".gltf", ".glb", ".stl", ".dae"
};

const std::set<std::string> kTextExtensions = {
    ".txt", ".md", ".cpp", ".hpp", ".c", ".h", ".py",
...
MediaType MediaService::DetectMediaType(const std::filesystem::path& path) {
    ...
    if (IsModelFile(path)) return MediaType::Model;
    if (IsTextFile(path)) return MediaType::Text;
    return MediaType::Unknown;
}

std::expected<ImageMetadata, std::string> MediaService::GetImageMetadata(
    ...
    try {
        ...
        spdlog::debug("MediaService::GetImageMetadata: {}x{}, {}-bit, {}",
                      meta.width, meta.height, meta.colorDepth, meta.format);
...
std::expected<std::vector<std::uint8_t>, std::string> MediaService::GetImageData(
    const std::filesystem::path& path) {
    if (!std::filesystem::exists(path)) {
        return std::unexpected("File not found: " + path.string());
    }
