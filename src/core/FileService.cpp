...
std::expected<DecodeResult, std::string> FileService::LoadFile(const std::filesystem::path& path) {
    spdlog::info("FileService::LoadFile: {}", path.string());

    if (!std::filesystem::exists(path)) {
        spdlog::error("FileService::LoadFile: file not found: {}", path.string());
        return std::unexpected("File not found: " + path.string());
    }

    std::ifstream file(path, std::ios::binary);
    if (!file.is_open()) {
        spdlog::error("FileService::LoadFile: cannot open file: {}", path.string());
    ...
    if (!decodeResult) {
        spdlog::warn("FileService::LoadFile: decode warning: {}", decodeResult.error());
        return std::unexpected(decodeResult.error());
    }

    spdlog::info("FileService::LoadFile: loaded {} bytes, encoding: {}", data.size(),
                 Encoding::EncodingName(decodeResult->detectedEncoding));

    return *decodeResult;
}

std::expected<void, std::string> FileService::SafeSave(const std::filesystem::path& path, std::string_view content,
                                                       TextEncoding encoding) {
    auto parentDir = path.parent_path();
    if (!parentDir.empty() && !std::filesystem::exists(parentDir)) {
        spdlog::error("FileService::SafeSave: directory does not exist: {}", parentDir.string());
        return std::unexpected("Directory does not exist: " + parentDir.string());
    }
    ...
    if (!encodeResult) {
        spdlog::error("FileService::SafeSave: encode failed: {}", encodeResult.error());
        return std::unexpected("Encoding failed: " + encodeResult.error());
    }

    std::ofstream outFile(tempPath, std::ios::binary);
    if (!outFile.is_open()) {
        spdlog::error("FileService::SafeSave: cannot create temp file: {}", tempPath.string());
        return std::unexpected("Cannot create temporary file");
    }

    outFile.write(reinterpret_cast<const char*>(encodeResult->bytes.data()),
    ...
    spdlog::info("FileService::SafeSave: saved successfully: {}", path.string());
    return {};
}

std::expected<void, std::string> FileService::SaveFile(const std::filesystem::path& path, std::string_view content,
                                                       TextEncoding encoding) {
    spdlog::info("FileService::SaveFile: {}", path.string());
    return SafeSave(path, content, encoding);
}

std::expected<void, std::string> FileService::DeleteFile(const std::filesystem::path& path) {
    spdlog::info("FileService::DeleteFile: {}", path.string());

    if (!std::filesystem::exists(path)) {
        spdlog::warn("FileService::DeleteFile: file does not exist: {}", path.string());
    ...
    if (ec) {
        spdlog::error("FileService::DeleteFile: remove failed: {}", ec.message());
        return std::unexpected("Failed to delete file: " + ec.message());
    }

    spdlog::info("FileService::DeleteFile: deleted successfully: {}", path.string());
    return {};
}
